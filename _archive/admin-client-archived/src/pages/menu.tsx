...
export default function MenuPage() {
  ...
  // Track cart count changes for latency analysis
  useEffect(() => {
    const currentCount = getTotalItems();
    console.log("🔄 [CART LATENCY] UI: Cart count changed to:", currentCount, "at:", new Date().toISOString());
    console.log("🔄 [CART LATENCY] UI: Current cart items:", cartItems);
  ...
  const handleAddToCart = (menuItem: MenuItem, customizations?: any, specialInstructions?: string, quantity?: number) => {
    ...
    console.log("🎯 [CART LATENCY] UI: handleAddToCart called at:", new Date().toISOString());
    console.log("🎯 [CART LATENCY] UI: Current cart count before add:", getTotalItems());

    addToCart(menuItem, customizations, specialInstructions, quantity);

    const uiCallTime = performance.now() - uiStartTime;
    console.log(`⚡ [CART LATENCY] UI: addToCart call took: ${uiCallTime.toFixed(2)}ms`);
  };
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              ...
              <button
                ...
                className="relative bg-orange-600 text-white p-3 rounded-2xl shadow-lg hover:bg-orange-700 transition-colors"
              >
