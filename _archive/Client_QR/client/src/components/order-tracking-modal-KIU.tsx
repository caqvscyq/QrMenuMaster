...
export function OrderTrackingModal({ isOpen, onClose }: OrderTrackingModalProps) {
  // Use the same session management as cart - no race conditions!
  const { sessionId, isLoading: isSessionLoading } = useCart();
  const scrollContainerRef = useRef<HTMLDivElement>(null);
  const [tableNumber, setTableNumber] = useState<string | null>(null);
  const [deskId, setDeskId] = useState<string | null>(null);
  
  // Extract table/desk info from URL parameters
  useEffect(() => {
  ...
  // Fetch orders by session ID with shorter refresh interval
  const { data: allOrders = [], isLoading } = useQuery<OrderWithItems[]>({
    queryKey: ["/api/orders", (window as any).__currentSessionId, tableNumber], // Use unified session ID
    queryFn: async () => {
      ...
      if (!currentSessionId) {
        console.warn("No global session ID available for order tracking");
        return [];
      }
      ...
      if (!newFormatPattern.test(currentSessionId) && !oldFormatPattern.test(currentSessionId)) {
        console.warn("Invalid session ID format:", currentSessionId);
        return [];
      }

      console.log(`✅ UNIFIED SESSION: Fetching orders for table ${tableNumber}, session:`, currentSessionId);
      const response = await apiRequest("GET", `/api/orders/session/${currentSessionId}`);
      if (!response.ok) {
        throw new Error(`Error fetching orders: ${response.statusText}`);
      }
      const data = await response.json();
      console.log(`Orders data fetched for table ${tableNumber}:`, data);

      // Additional client-side validation to ensure orders belong to this session
      const validatedOrders = data.filter((order: OrderWithItems) => {
        if (order.sessionId !== currentSessionId) {
          console.warn(`Order ${order.id} has mismatched session ID:`, order.sessionId, 'vs', currentSessionId);
          return false;
        }
        return true;
      });
    ...
    enabled: isOpen && !!(window as any).__currentSessionId && !isSessionLoading && !!tableNumber, // Wait for unified session to be loaded
    refetchInterval: 1000, // Refresh every 1 second when modal is open
  ...
  // Filter orders by current table number with strict validation
  const orders = allOrders.filter(order => {
    // Get the unified session ID for consistent validation
    const currentSessionId = (window as any).__currentSessionId;

    // First, ensure the order belongs to the current session
    if (order.sessionId !== currentSessionId) {
      console.warn(`Filtering out order ${order.id} with wrong session ID:`, order.sessionId);
      return false;
    }

    // If we have a table number, only show orders for this table
    if (tableNumber && tableNumber !== "unknown") {
      if (order.tableNumber && order.tableNumber === tableNumber) {
        return true;
      }
      // If order doesn't have table number but session ID contains the table, allow it
      if (!order.tableNumber && currentSessionId.includes(`session-${tableNumber}-`)) {
        return true;
      }
      return false;
    }
    ...
    // If no table or desk ID, only show orders that match the session ID
    return order.sessionId === currentSessionId;
  });

  useEffect(() => {
    if (isOpen) {
      console.log("OrderTrackingModal opened with sessionId:", sessionId);
      console.log("Current table number:", tableNumber);
      console.log("Current desk ID:", deskId);
      console.log("Filtered orders:", orders);
    }
  }, [isOpen, sessionId, orders, tableNumber, deskId]);
