...
export interface IStorage {
  ...
  getMenuItemsByCategory(category: number): Promise<MenuItem[]>;
...
export class MemStorage implements IStorage {
  ...
  private initializeMenuItems() {
    ...
    sampleItems.forEach(item => {
      const menuItem: MenuItem = { ...item, id: this.currentMenuItemId++, shopId: 1 };
      this.menuItemsMap.set(menuItem.id, menuItem);
    });
  }

  async getMenuItems(): Promise<MenuItem[]> {
    return Array.from(this.menuItemsMap.values()).filter(item => item.shopId === 1);
  }

  async getMenuItemsByCategory(category: number): Promise<MenuItem[]> {
    const allItems = Array.from(this.menuItemsMap.values());
    return allItems.filter(item => item.categoryId !== null && item.categoryId === category && item.shopId === 1);
  }
  ...
  async getCartItems(sessionId: string): Promise<(CartItem & { menuItem: MenuItem })[]> {
    ...
    }).filter(item => item.menuItem.shopId === 1);
  }
