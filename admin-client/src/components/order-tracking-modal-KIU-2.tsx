...
export function OrderTrackingModal({ isOpen, onClose }: OrderTrackingModalProps) {
  ...
  const { data: allOrders = [], isLoading } = useQuery<OrderWithItems[]>({
    queryKey: ["/api/orders", (window as any).__currentSessionId],
    queryFn: async () => {
      ...
      console.log("✅ UNIFIED SESSION: Fetching orders for session:", currentSessionId);
    ...
    enabled: isOpen && !!(window as any).__currentSessionId && !isSessionLoading, // Wait for unified session to be loaded
