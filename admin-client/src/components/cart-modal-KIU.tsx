...
export function CartModal({ isOpen, onClose, onOpenOrderTracking }: CartModalProps) {
  ...
  // Extract table number from URL parameters with validation
  useEffect(() => {
  ...
  // Debug log with table validation
  console.log(`Cart Items in Modal for table ${tableNumber}:`, cartItems);

  // Validate that cart items belong to this table's session
  useEffect(() => {
    if (cartItems.length > 0 && tableNumber !== "--" && tableNumber !== "unknown") {
      const sessionKey = `cart-session-id-${tableNumber}`;
      const expectedSessionId = localStorage.getItem(sessionKey);

      if (expectedSessionId) {
        // This is just a client-side check for debugging
        console.log(`Expected session ID for table ${tableNumber}:`, expectedSessionId);
      }
    }
  ...
  const handleCreateOrder = async () => {
    try {
      // Validate table number before creating order
      if (!tableNumber || tableNumber === "--" || tableNumber === "unknown") {
        toast({
          title: "訂單失敗",
          description: "無效的桌號，請重新掃描QR碼",
          variant: "destructive",
        });
        return;
      }
      ...
      console.log(`Creating order for table ${tableNumber} with ${cartItems.length} items`);
      const order = await createOrder(tableNumber); // Use validated table number
      ...
      toast({
        title: "訂單已送出！",
        description: `桌號 ${tableNumber} - 訂單編號 #${order.id} - 感謝您的訂購，餐點將為您準備`,
      });
    } catch (error) {
