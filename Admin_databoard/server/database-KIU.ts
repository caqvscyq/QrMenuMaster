...
import {
  users, categories, menuItems, orders, orderItems, shops, desks, cartItems,
...
export class DatabaseStorage implements IStorage {
  ...
  async completeAndPayDeskOrders(deskId: number): Promise<Order[]> {
    try {
      ...
      try {
        // Get all session IDs for orders at this table
        const sessionIds = activeOrders
          .map(order => order.sessionId)
          .filter(sessionId => sessionId !== null);

        console.log(`[DEBUG] Found ${sessionIds.length} unique sessions for desk ${deskId}`);

        // Clear cart items for all sessions at this table
        if (sessionIds.length > 0) {
          const deletedCartItems = await db
            .delete(cartItems)
            .where(inArray(cartItems.sessionId, sessionIds))
            .returning();

          console.log(`[DEBUG] Cleared ${deletedCartItems.length} cart items for desk ${deskId}`);
        }
